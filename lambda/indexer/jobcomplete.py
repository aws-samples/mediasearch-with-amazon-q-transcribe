...
from common import INDEX_ID, DS_ID, QBUSINESS_APPLICATION_ID, RETRIEVER_TYPE
from common import S3, TRANSCRIBE, INDEX
from common import stop_index_sync_job_when_all_done
...
def get_qbusiness_type_and_value(key, value):
    qbusiness_type = ""
    qbusiness_value = value
    if type(value) is int:
        qbusiness_type = "longValue"
    elif type(value) is list:
        qbusiness_type = "stringListValue"
        qbusiness_value = list(map(lambda x: str(x), value))
    elif type(value) is str:
        qbusiness_type = "stringValue"
        if iso8601_datetime(value):
            qbusiness_type = "dateValue"
            qbusiness_value = iso8601_datetime(value)
    else:
        logger.error(f"Metadata attribute {key} has invalid type {type(value)} - convert to string")
        qbusiness_type = "stringValue"
        qbusiness_value = str(value)
    return [qbusiness_type, qbusiness_value]
...
def put_document(dsId, indexId, s3url, item, text):
    ...
    documents = [document]
    logger.info("INDEX.batch_put_document: " + json.dumps(documents, default=str)[0:1000] + "...")
    result = INDEX.batch_put_document(
        IndexId = indexId,
        Documents = documents
    )
...
def get_document_qbusiness(dsId, indexId, s3url, item, text):
    bucket, key, file_name = parse_s3url(s3url)
    region = get_bucket_region(bucket)
    document = {
        "id": s3url,
    ...
    if metadata.get("Attributes"):
        logger.info(f"Set 'Attributes'")
        metadata_attributes = get_metadata_qbusiness_attributes(metadata)
        if any('ytsource' in d.values() for d in metadata_attributes):
            logger.info("YouTube Media being indexed")
            remove = '_source_uri'
            newAttributeList=[]
            for ele in document['attributes']:
                if '_source_uri' not in ele['name']:
                    newAttributeList.append(ele)
            document['attributes'] = newAttributeList
    ...
    ### TODO: Fix Access Control List mapping to Qbusiness API
    #if metadata.get("AccessControlList"):
    #    logger.info(f"Set 'AccessControlList'")
    #    document["AccessControlList"] = metadata['AccessControlList']
    return document

def put_document_qbusiness(dsId, indexId, applicationId, s3url, item, text):
    logger.info(f"put_document_qbusiness(dsId={dsId}, indexId={indexId}, applicationId={applicationId}, s3url={s3url}, text='{text[0:100]}...')")
    document = get_document_qbusiness(dsId, indexId, s3url, item, text)
    documents = [document]
    logger.info("INDEX.batch_put_document: " + json.dumps(documents, default=str)[0:1000] + "...")
    result = INDEX.batch_put_document(
...
def get_metadata_qbusiness_attributes(metadata):
    qbusiness_metadata_attributes=[]
    meta_attributes = metadata.get("Attributes")
    if meta_attributes:
        if type(meta_attributes) is dict:
            for key, value in meta_attributes.items():
                ...
                logger.info(reserved_attributes)
                if key not in reserved_attributes:
                    qbusiness_type, qbusiness_value = get_qbusiness_type_and_value(key, value)
                    qbusiness_attr = {
                        'name': key,
                        'value': {
                            qbusiness_type: qbusiness_value
                        }
                    }
                    qbusiness_metadata_attributes.append(qbusiness_attr)
                else:
                    logger.error(f"Metadata may not override reserved attribute: {key}")
        else:
            logger.error(f"Metadata 'Attributes' is not dict type: {type(meta_attributes)}")
        ...
        logger.info(f"Metadata does not contain 'Attributes'")
    return qbusiness_metadata_attributes
        ...
        if job_status == "FAILED":
            ...
            try:
                ...
                if (RETRIEVER_TYPE == 'Kendra'):
                    ...
                    logger.info("** Index transcription document in QBusiness Index **")
                    put_document_qbusiness(dsId=DS_ID, indexId=INDEX_ID, applicationId=QBUSINESS_APPLICATION_ID, s3url=media_s3url, item=item, text=text)
                # Update sync_state
                put_file_status(
